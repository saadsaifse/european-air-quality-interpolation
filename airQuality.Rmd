...
```{r, echo = FALSE}
knitr::opts_chunk$set(collapse=TRUE, comment="##", fig.retina=2, fig.path = "../plots/airquality-")
```
...
stations = read.csv("data/AirBase_v8_stations.csv", sep = "\t", stringsAsFactors = FALSE)
...
## Downlaoding Time Series Dataset

The following script fetches and stores all air quality time series `.csv` files from the stations that exist in the specified radius of the provided point. It does so by using `https://fme.discomap.eea.europa.eu/fmedatastreaming/AirQualityDownload/AQData_Extract.fmw` link from European Environmental Agency (EEA) and dynamically adding the appropriate query pramaters in the base URL. The code is now synchronous and could tak longer if the specified radius is too big or the data is large but an asynchronous loop can be used to overcome this challenge using `doParallel` R package.

Define if data needs to be downloaded
...
```{r}
files = list.files("data", pattern = "DE_5*", full.names = TRUE)
...
```{r}
library(tidyverse)
read.csv("data/AirBase_v8_stations.csv", sep = "\t", stringsAsFactors = FALSE) %>%
  as_tibble %>% 
